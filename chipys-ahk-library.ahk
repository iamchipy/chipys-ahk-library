...
{
	__New(x := 100, y := 100, button := "RButton") {
		this.x := x
		this.y := y
		this.button := button
...
{
	; Property to store the function reference.
	; This will hold the Func object, a lambda, or a bound function.
	; Initialized to an empty string or null to indicate no function is set.
	stored_function := Map("default", "")
	state := false
	success := false

	; Method to accept the function reference.
	; 'funcRef' can be a Func object, a lambda, or a bound function.
	; It's important to pass the function itself, not its name as a plain string,
	; if you want to pass arguments later or ensure it's a callable object.
	store_as(funcRef, index := this.__Class)
	{
		; Validate if the passed parameter is actually callable.
		; IsObject() checks if it's a Func object, a lambda, or a bound function.
		; IsCallable() ensures it can be invoked.
		if !IsObject(funcRef) || !funcRef.HasMethod()
		{
			error_str := "Invalid method/func:`n"
			if this.HasOwnProp("name")
				error_str .= this.name
			error_str .= ".store_as() [EXPECTED: funcObject/lambda/boundFunc] and received: " Type(funcRef)
			MsgBox error_str
			return false
		}

		this.stored_function[index] := funcRef
	...
	execute_stored_function(params*)
	{
		; debug
		; MsgBox a2s(params, " exec_params::")

		; Build out index value from Params by checking for a value
		index := this.__Class
		if (params.length > 0) {
			if this.stored_function.Has(params[1]) {
				; sort the value and remove from param list to not overload func
				index := params[1]
				params.RemoveAt(1, 1)
			}
		}
		...
		if Type(index) != "String"
			throw "execute_stored_function received invalid(expected string but got index=>" Type(index) ")"

		; debugging
		; MsgBox "execute_stored_function() received types:`nindex:" type(index) "`nstored:" type(this.stored_function) "`nparams:" type(params)
		; MsgBox "execute_stored_function() received:`nindex:" index "`nparams:" params.Length
		; for p in params
		; 	MsgBox p.length

		; Exit case where nothing is stored
		if (this.stored_function.Has(index) && this.stored_function[index] == "")
		{
			MsgBox "Aborted call to empty/blank stored_function['" index "'] "
			return 0 ; Return empty if no function is set
		}

		; Call the stored function using the .Call() method.
		; This allows passing an arbitrary number of parameters.
		; If the stored function doesn't expect parameters, they will be ignored.
		try
		{

			; MsgBox "Calling '" index "' with " a2s(params)
			result := this.stored_function[index].Call(params*)
			; MsgBox "Executed stored function. Result: " result
			return result
		}
		catch as e
		{
			MsgBox "Error executing stored function['" index "']:`nMsg: " e.Message "`nWhat: " e.What "`nExtra: " e.Extra "`nType: " Type(e) "`na2s: " a2s(params)
			return ""
		}
	}
}
...
; Expects:
; Name - String for self id
; eval_func - Func object, a lambda, or a bound function
class Condition extends DeferredCaller {
	__new(name, eval_func) {
		this.name := name
		this.store_as(eval_func, index := this.__Class)
	}

	; Calls the stored function to evaluate the state/condition based on the function's response.
	; Result is kept in this.state and also returned
	eval(variables_to_evalate*) {

		; MsgBox a2s(variables_to_evalate, ".eval>>")
		this.state := this.execute_stored_function(this.__Class, variables_to_evalate*)
		return this.state
	}
}

; Stores an action function that can be called as a function but is wrapped with a condition
; Expects:
;  - Name
;  - action_func - Func object, a lambda, or a bound function
;  - pre_condition - Condition object to evaluate BEFORE act
class Action extends DeferredCaller {
	__new(name, action_func := 0, pre_condition := 0) {
		this.name := name
		this.store_as(action_func, this.__Class)

		if pre_condition && Type(pre_condition) != "Condition"
			throw "Invalid param:: pre_condition expected Condition but received " Type(pre_condition)
		this.pre_condition := pre_condition
	}

	is_ready(params*) {
		; check if we have a condition to use for evaluation
		if this.pre_condition && this.pre_condition.HasMethod("execute_stored_function") {
			return this.pre_condition.eval(params*)
		}
		; without a condition we assume none and thus always ready
		return true
	}

	; Allows Action object to be called as if it's an alias for it's stored action wrapped in a condition
	; NEGATIVE return values are failure to meet .is_ready (.pre_condition check)
	Call(params*) {
		if !this.is_ready(params*)
			return 0
		; MsgBox "approved now calling Action:: with " a2s(params)

		; do NOT pass along params (for now) since we are passing CONDITION check changing action
...
class RichPixel {
	...
	; this is here to allow legacy direct ref like an array
	__Item[i] {
		get {
			switch i {
				case "default":
	...
	_strip_hex_prefix(in_str) {
		if type(in_str) == "Integer" {
			; msgbox in_str "->" format("{:x}",in_str)
			in_str := format("{:x}", in_str)
		}
		; msgbox in_str "->" format("{:x}",in_str)
		if inStr(in_str, "0x")
			in_str := StrSplit(in_str, "0x")[2]
		; msgbox in_str
		return in_str
	}

	_split_rgb(rgb_hex_str) {
		if Type(rgb_hex_str) != "String"
			throw "_split_rgb::expected string but received:" Type(rgb_hex_str)
		if StrLen(rgb_hex_str) != 6
			throw "invalid RGB String(len) -> '" rgb_hex_str "'(" StrLen(rgb_hex_str) ")"
		this.r := SubStr(rgb_hex_str, 1, 2)
		this.g := SubStr(rgb_hex_str, 3, 2)
		this.b := SubStr(rgb_hex_str, 5, 2)
	...
	_hex2int(hex_str) {
		; f:="FF"
		; MsgBox Format("{:i},{:i},{:i}","0x" f,0xFF,0xFF)

		return format("{:i}", "0x" hex_str)
	}
	...
	_color_difference(value_to_match, total_vs_channel := "channel") {
		a_rgb_array := this._split_rgb(this._strip_hex_prefix(this.color))
		b_rgb_array := this._split_rgb(this._strip_hex_prefix(value_to_match))
		d_rgb_array := [0, 0, 0]

		loop a_rgb_array.Length {
			log("SPAM: RichPixel._color_difference() -> RGB-LOOP  [a " this._hex2int(a_rgb_array[a_index]) " / b " this._hex2int(b_rgb_array[a_index]) "]")
			d_rgb_array[a_index] := abs(this._hex2int(a_rgb_array[a_index]) - this._hex2int(b_rgb_array[a_index]))
		}
		largested_difference := d_rgb_array[1]
		if largested_difference < d_rgb_array[2]
			largested_difference := d_rgb_array[2]
		if largested_difference < d_rgb_array[3]
			largested_difference := d_rgb_array[3]

		log("SPAM: RichPixel._color_difference() -> largested_difference  [ " largested_difference "]")
		return largested_difference
	}

	is(value_to_match) {
		return this.match(value_to_match)
	}
	...
	; returns bool
	match(value_to_match, variation := 0) {
		if variation {
			dif := this._color_difference(value_to_match)
			log("SPAM: RichPixel.Match() -> " dif "  [" this._int2hex(value_to_match) "/" this.color "]")
			if abs(dif) < variation {
				return True
			}
		}
		return (this.color == value_to_match)
	}
...
class MetaInfo {
	__New(app_version := "0.0.1",
		...
		this.target_window_name := target_window_name
...
class ThreeButtonMenu {
	...
	_gui_build() {
		...
		this.gui_obj.setfont("ccccc00 s" GUI_FONT_SIZE * 0.8 " w400 q3", "Terminal")			;incase of missing font
	...
	_gui_update(items := 0, display_time := 5000) {
		...
		this.ctr_p.setfont("s" GUI_FONT_SIZE * 0.8)
		this.ctr_c.setfont("s" GUI_FONT_SIZE * 1.2)
		this.ctr_n.setfont("s" GUI_FONT_SIZE * 0.8)
		if (items != 0) {
...
class ConfigManagerTool {
	...
	__new(file_name := 0, custom_section := 0, client_name := 0, meta_info := 0, tray_builder := 0) {
		;sets default for custom section is != 0
		((meta_info) ? (this.meta_info := meta_info) : (this.meta_info := meta_info))
		((tray_builder) ? (this.tray_builder := tray_builder) : (this.tray_builder := tray_builder))
	...
	_load_meta_info() {
		; guard case for no tray object
		if !this.meta_info
			return false

		this.file_name := this.meta_info.config_path
		this.client_name := this.meta_info.target_window_name
	}

	; creates the tray menu item for accessing option (ONLY if trayBuilder provided)
	; DISCONTINUED FOR NOW DUE TO 'this' not being something i can cache in the scope of TrayMenu and still instance reference
	_create_tray_entry() {
		; guard case for no tray object
...
; A simple null endpoint for tray menu and other things that need an blank endpoint
null_func(*) {
	return false
}

; Simple function to support Tray and other references to reload as a function
reload_alias(*) {
	reload
}
...
baseline_sleep_time(ms_to_sleep_for := 1, halting_flag := false, base_delay_time := 100) {
	...
	if ms_to_sleep_for < base_delay_time {
		...
		sleep ms_to_sleep_for * Random(0.9 * base_delay_time, 1.1 * base_delay_time)
		return
	}

	roughened_time_to_sleep := Random(0.9 * ms_to_sleep_for, 1.1 * ms_to_sleep_for)
